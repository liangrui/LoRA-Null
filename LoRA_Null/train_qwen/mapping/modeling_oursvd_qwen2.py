...
from .configuration_oursvd_qwen2 import CovSVDQwen2Config
...
class CovSVDLinear(nn.Module):
    """CovSVD (LoRA-Null) adapter linear layer.

    Structure: BLinear(in -> rank) + ALinear(rank -> out) + weight_residual(out x in, frozen)
    Forward: y = ALinear(BLinear(x)) + F.linear(x, weight_residual)
    """
...
class CovSVDQwen2ForCausalLM(Qwen2ForCausalLM):
    config_class = CovSVDQwen2Config

    def __init__(self, config: CovSVDQwen2Config):
